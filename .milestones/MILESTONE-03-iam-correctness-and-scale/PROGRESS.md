...
## M03-T14 — Seed `--members N` and record the timings

- **Status**: in-progress
- **Date**: 2026-08-15
- **Approach**: M03-T06 and T07 measured 100k with throwaway scripts. Make that
  reproducible: a `--members N` mode on the seed script, then record 1k / 10k /
  100k timings here so the numbers are committed rather than quoted.
- **Weight**: not heavy. No review file: the artifact is the numbers below.
- **Changed**: `apps/backend/scripts/seed.ts` (`--members N`),
  `apps/backend/scripts/measure-members.ts` (new), `apps/backend/package.json`
  (`measure:members`).
- **Verified**: `moon check --all` — 23 tasks green; `bunx knip` clean.

### Measured — `bun run measure:members`

Median of five calls after one warm-up, against an in-memory SQLite database on
this machine. Budget is M03-T06's exit criterion: **200 ms**.

| Members | Seed | page 1 | sorted by name | filtered search | role facet | deep page |
|---|---|---|---|---|---|---|
| 1,001 | 31 ms | **1.4 ms** | 1.2 ms | 1.3 ms | 1.4 ms | 0.8 ms |
| 10,001 | 227 ms | **8.1 ms** | 5.0 ms | 6.7 ms | 7.9 ms | 3.2 ms |
| 100,001 | 2,146 ms | **73.3 ms** | 48.9 ms | 73.5 ms | 84.0 ms | 31.8 ms |

Every measurement is inside the budget, and the script exits non-zero if one is
not — so re-running it is a check, not a report to read.

Growth is roughly linear in rows for the scanning queries (1k → 100k is ×100
data for ×52 on page 1), which is what a `LIKE` filter and a `COUNT(*)` over an
un-indexed join predict. That is the shape M07 will change, not this milestone.

- **Notes**: the timings live here rather than in the test suite. Seeding
  100,000 rows takes ~2 s and the assertion is a latency budget, which makes a
  flaky gate on a shared CI runner — `M03-T06`'s journal already flagged that
  and handed the reproducibility to this task. `measure-members.ts` closes it:
  the numbers can be regenerated by anyone with one command.

  The old `listOrgMembers` cannot be measured at 100k for comparison, because it
  does not return — it throws `SQLite query expected 34464 values, received
  100000`. That figure is in M03-T06's entry.
- **Divergence**: the task named only `seed.ts`. A `--members N` flag on the seed
  makes a large organization, but measuring it still needed a second throwaway
  script — exactly what T06 and T07 had already written twice. `measure-members.ts`
  replaces both. knip flagged it as an unused file, correctly: it is reachable
