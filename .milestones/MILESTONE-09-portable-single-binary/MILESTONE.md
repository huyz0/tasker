...
- [x] `moon run backend:build-standalone` produces one file that, copied to an
      empty directory on a machine with no toolchain, serves the SPA and the API.
- [x] The binary creates and migrates its own SQLite database on first run.
- [x] Full-text search works in the binary with no external service.
- [x] Configuration is read from a `.env` beside the binary and from flags.
- [x] CI builds the binary, runs it in a temporary directory, and asserts the
      health endpoint and the index page respond.
- [x] The literal placeholder HTML currently served at `/` is gone.
- [x] The in-process transport is either implemented or deleted, with an ADR
...
- [x] **M09-T01** — Embed migrations in the binary instead of reading
...
- [x] **M09-T02** — Build the GUI as part of the standalone build and embed
      `dist/` into the binary.
      - Files: `apps/backend/package.json`, `apps/backend/moon.yml`
      - Verify: the binary contains the asset bundle.

- [x] **M09-T03** — Serve the embedded assets with correct content types, cache
...
- [x] **M09-T04** — Decide the in-process transport question in an ADR: implement
...
- [x] **M09-T05** — Add `--port`, `--db`, `--open` and `--seed` flags with Zod
...
- [x] **M09-T06** — Make standalone mode work without `ENABLE_TEST_LOGIN`: a
...
- [x] **M09-T07** — Add a CI smoke job: build, run in a temp directory, assert
...
- [x] **M09-T08** — Produce release artifacts for linux, macOS and Windows.
      - Files: `.github/workflows/release.yml`
      - Verify: a tagged build attaches three binaries.

- [x] **M09-T09** — Write the standalone quickstart.
