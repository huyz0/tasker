...
/** Registers AssignTask/UnassignTask and records every request each receives. */
function withAssignUnassign() {
  const assignRequests: any[] = [];
  const unassignRequests: any[] = [];
  mockRpc(TaskService, 'AssignTask', (body) => {
    assignRequests.push(body);
    return { success: true };
  });
  ...
  return { assignRequests, unassignRequests };
}
...
function withListAgents(response: object) {
  const requests: any[] = [];
...
describe('AssigneePicker', () => {
  ...
  it('asks the server to filter, rather than fetching everyone and filtering here', async () => {
    const memberRequests = withListMembers({ members: [], page: { totalCount: 0 } });
    const agentRequests = withListAgents({ agents: [], page: { totalCount: 0 } });
    renderPicker([]);
    ...
    await waitFor(() => expect(memberRequests).toContainEqual(
  ...
  it('says how many matched when it is showing only some', async () => {
    withListMembers({ members: [{ userId: 'u-1', name: 'Ada Lovelace', email: 'a@x.test' }], page: { totalCount: 100001 } });
    withListAgents({ agents: [], page: { totalCount: 0 } });
    renderPicker([]);
    await openPicker();
  ...
  it('assigns a person', async () => {
    const { assignRequests } = withAssignUnassign();
    renderPicker([]);
    await openPicker();
    fireEvent.click(await screen.findByRole('button', { name: 'Ada Lovelace' }));
    await waitFor(() => expect(assignRequests).toContainEqual({ taskId: 'task-1', userId: 'u-1' }));
  });
  ...
  it('assigns an agent', async () => {
    ...
    await waitFor(() => expect(assignRequests).toContainEqual({ taskId: 'task-1', agentId: 'a-1' }));
  });
  ...
  it('removes an assignee, naming who is being removed', async () => {
    const { unassignRequests } = withAssignUnassign();
    renderPicker([{ userId: 'u-1', agentId: '', name: 'Ada Lovelace' }]);
    fireEvent.click(await screen.findByLabelText('Remove Ada Lovelace from this task'));
    await waitFor(() => expect(unassignRequests).toContainEqual({ taskId: 'task-1', userId: 'u-1' }));
  });
  ...
  it('removes an agent by its agent id, not as a user', async () => {
    ...
    await waitFor(() => expect(unassignRequests).toContainEqual({ taskId: 'task-1', agentId: 'a-1' }));
  });

  it('reports a failed assignment instead of silently doing nothing', async () => {
    mockRpcError(TaskService, 'AssignTask', 'permission_denied', 'permission denied');
    renderPicker([]);
    await openPicker();
    fireEvent.click(await screen.findByRole('button', { name: 'Ada Lovelace' }));
  ...
  it('reports a failed removal and keeps the row', async () => {
    mockRpcError(TaskService, 'UnassignTask', 'unknown', 'nope');
    renderPicker([{ userId: 'u-1', agentId: '', name: 'Ada Lovelace' }]);
    fireEvent.click(await screen.findByLabelText('Remove Ada Lovelace from this task'));
