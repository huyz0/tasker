...
- **architecture-create-auto**: Use an **Advanced Reasoning Model** tier.
- **ux-design-auto**: Use a **Multimodal / Fast Creative Model** tier.
- **qa-plan-define-auto**: Use a **Standard / Fast Reasoning Model** tier.
...
3. **Parallel Execution (Architecture & UX):** If applicable, spawn concurrent sub-agents to run `.agents/skills/architecture-create-auto/SKILL.md` (using Advanced Reasoning Model) and `.agents/skills/ux-design-auto/SKILL.md` (using Multimodal Model) IN PARALLEL. Ensure you `await` both of their completions before proceeding. The Orchestrator forwards the `EPIC.md` context without pre-loading the rest of the codebase.
4. **Test Plan Execution:** ONLY ONCE Architecture and UX are fully complete and returned, spawn a sub-agent to execute `.agents/skills/qa-plan-define-auto/SKILL.md` (using Standard Reasoning Model) for this epic.
6. **Report & Next Steps:**
