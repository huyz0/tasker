...
4. **Epic registry**: Scan `.milestones/*/MILESTONE.md` using `grep_search` to extract frontmatter (`status`, `title`). Also read `.archive/EPICS-HISTORY.md` (if it exists). Also scan `.milestones/*/reviews/SECURITY-REVIEW-*.md` and `.archive/epics/*/reviews/SECURITY-REVIEW-*.md` for past security findings.
...
- `.milestones/*/reviews/SECURITY-REVIEW-*.md` — Past security review findings (if any).
