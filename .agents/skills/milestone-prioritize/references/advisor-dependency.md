...
3. **Epic registry**: Scan `.milestones/*/MILESTONE.md` using `grep_search` to extract frontmatter (`status`, `title`, `created_at`) and `## Dependencies` sections. Also read `.archive/EPICS-HISTORY.md` (if it exists) to include completed and archived epics. Build a full registry: `{ id, title, status, dependencies[] }`.
...
1. Build a **dependency graph** from all milestone `depends_on` frontmatter and roadmap item relationships.
...
1. Build a **dependency graph** from all milestone `depends_on` frontmatter.
...
- `.milestones/*/MILESTONE.md` — `depends_on` of each milestone. Build the dependency graph.
