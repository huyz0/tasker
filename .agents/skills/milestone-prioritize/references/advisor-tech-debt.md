...
2. **Standards**: Invoke the **context-inject-auto** skill to load `coding-standard.md`, `testing-standard.md`, `observability-standard.md`.
...
4. **Epic registry**: Scan `.milestones/*/MILESTONE.md` using `grep_search` to extract frontmatter (`status`, `title`). Also read `.archive/EPICS-HISTORY.md` (if it exists). Also scan for review findings in `.epics/*/reviews/` and `.archive/epics/*/reviews/`.
...
- `.milestones/*/MILESTONE.md` — Delivered milestones and their review findings (architecture reviews, code reviews).
