...
3. **Epic registry**: Scan `.milestones/*/MILESTONE.md` using `grep_search` to extract frontmatter (`status`, `title`). Also read `.archive/EPICS-HISTORY.md` (if it exists) to include completed and archived epics. Identify delivered milestones to understand what's already built.
4. **Codebase survey**:
...
3. Identify items with highest **reuse potential** from delivered milestones.
...
2. **Reuse potential**: How much of the candidate's scope is already partially implemented by delivered milestones?
...
- `.milestones/*/MILESTONE.md` — Delivered milestone scopes to determine what's already built.
