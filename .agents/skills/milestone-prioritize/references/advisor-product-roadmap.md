...
3. **Epic registry**: Scan `.milestones/*/MILESTONE.md` frontmatter (`id`, `title`, `status`, `depends_on`, `surfaces`) and `.milestones/STATE.md` for the ledger. Also read `.archive/EPICS-HISTORY.md` for scope delivered under the retired epic system. Build a registry: `{ id, title, status, dependencies[] }`.
4. **Domain data**: Read `.specs/product/roadmap.md` (primary source), `.specs/product/mission.md`, and cross-reference against the milestone registry.

## Candidate Discovery

In **Round 1**, you MUST propose candidate work items from your domain perspective:

1. Compare every line item in `roadmap.md` against delivered milestone scopes.
2. For each roadmap item, classify:
   - **Fully covered** by a `status: done` milestone → exclude
...
5. **Coverage gap**: Which Phase 1 items are NOT yet covered by delivered milestones? Uncovered Phase 1 items are urgent.
...
- `.milestones/*/MILESTONE.md` frontmatter and titles — To determine which roadmap items are already covered.
...
| 3 | Phase 1 item partially covered by delivered milestones, or inferred from roadmap |
