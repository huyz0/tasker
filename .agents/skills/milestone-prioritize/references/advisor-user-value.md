...
2. **Standards**: Invoke the **context-inject-auto** skill to load `ui-ux-standard.md`.
...
4. **Epic registry**: Scan `.milestones/*/MILESTONE.md` using `grep_search` to extract frontmatter (`status`, `title`, `created_at`). Also read `.archive/EPICS-HISTORY.md` (if it exists). Build a registry: `{ id, title, status }`.
