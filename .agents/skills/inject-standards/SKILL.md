...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `.specs/standards/` (not in a subfolder).
...
2. **Creating a Skill/Workflow** — Building an `.agents/skills/` or `.agents/workflows/` file
...
- If conversation clearly mentions creating a skill, editing `.agents/skills/` or `.agents/workflows/`, or building a reusable procedure → **Creating a Skill/Workflow**
...
2. **Skill/Workflow** — Output file references to include in a skill or workflow you're building
...
Read `.specs/standards/index.yml` to get the list of available standards and their descriptions.
...
#### Scenario: Creating a Skill/Workflow

First, use AskUserQuestion to determine how to include the standards:

```
How should these standards be included in your skill/workflow?

1. **References** — Add @ file paths that point to the standards (keeps it lightweight, standards stay in sync)
2. **Copy content** — Paste the full standards content into it (self-contained, but won't update if standards change)
...
```
Be sure to include references to the following standards files in the appropriate location in the file(s) that make up this skill or workflow:

@.specs/standards/api/response-format.md
@.specs/standards/api/error-handling.md
@.specs/standards/global/naming.md
...
```
Include the following standards content in your skill/workflow:
...
### Step 6: Surface Related Skills/Workflows (Conversation scenario only)

When in conversation scenario, check if `.agents/skills/` or `.agents/workflows/` exists and contains related procedures:

```
Related Workflows/Skills you might want to use:
- create-api-endpoint — Scaffolds new API endpoints following these standards
```

Don't invoke skills automatically — just surface them for awareness. I (Antigravity) can run them if the user asks.
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `.specs/standards/api/`
- **Folder/file** — `api/response-format` → inject `.specs/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `.specs/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `.specs/standards/naming.md`
