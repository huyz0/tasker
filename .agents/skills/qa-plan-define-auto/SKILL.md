...
- CRITICAL ID FORMATTING: ALWAYS output files into a subdirectory exactly formatted as `.test-plans/TEST-PLAN-<4-digit-id>-<kebab-case-title>/`. The ID MUST be strictly zero-padded to 4 digits (e.g. `TEST-PLAN-0014-...` not `TEST-PLAN-14-...`). DO NOT use the epic's folder name format.
