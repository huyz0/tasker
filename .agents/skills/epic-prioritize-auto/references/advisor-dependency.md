...
3. **Epic registry**: Scan `.epics/*/EPIC.md` using `grep_search` to extract frontmatter (`status`, `title`, `created_at`) and `## Dependencies` sections. Build a full registry: `{ id, title, status, dependencies[] }`.
