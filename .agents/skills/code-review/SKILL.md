...
- ALWAYS resolve the review output path and filename format using `.specs/product/workflow.yml` `reviews.config.project_files.path` and `reviews.config.project_files.name_templates.code`. Find the next highest version number [N].
...
3. **Analyze:** Read `.specs/standards/coding-standard.md` and review the code based on the developer's highlighted areas.
4. **Determine Version:** Check `.epics/EPIC-<id>/reviews/` for existing `CODE-REVIEW-v*.md` files. Increment version.
5. **Report:** Generate the review document at the configured `workflow.yml` path and update `EPIC.md` `reviews.code` status. Ask if they want you to help apply the suggestions.
