...
Establish foundational product documentation through an interactive conversation. Creates mission, roadmap, architecture, and tech stack files in `.specs/product/`.
...
Check if `.specs/product/` exists and contains any of these files:
- `mission.md`
- `roadmap.md`
- `architecture.md`
- `tech-stack.md`
...
First, check if `.specs/standards/global/tech-stack.md` exists.
...
### Step 5: Establish Architecture (for architecture.md)

Use AskUserQuestion:

```
Let's outline the core system architecture.

**What are the main architectural patterns and deployment targets?**
(e.g., Microservices, Modular Monolith, CQRS, DDD, AWS, Kubernetes, etc.)

(Provide a high-level overview of the architectural intent and where it lives)
```
...
(Describe critical performance limits, scaling expectations, security needs, or latency targets)
```

### Step 6: Generate Files

Create the `.specs/product/` directory if it doesn't exist.
...
> Note: For architectural guidelines, deploy configurations and principles, see `architecture.md`.
...
[Insert overview of system interactions based on their answers]

## Patterns & Intents
[Insert explicit patterns like Event-Driven, DDD, CQRS, etc. - based on Step 5]

## Deployment
[Insert infrastructure intent - based on Step 5]

## Non-Functional Requirements (NFRs)
[Insert performance and scaling constraints - based on Step 5 responses]
```
