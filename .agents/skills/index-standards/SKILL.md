...
1. List all `.md` files in `.specs/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `.specs/standards/` (not in a subfolder). Do not create an actual folder named "root".

### Step 2: Load Existing Index

Read `.specs/standards/index.yml` if it exists. Note which entries already have descriptions.
...
Generate `.specs/standards/index.yml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `.specs/standards/` (not in subfolders).
