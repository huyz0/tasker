...
description: Autonomously evaluates uncovered roadmap items using an 8-advisor council model, scoring candidates across product roadmap, implementation state, technical debt, dependency order, user value, test coverage, security, and build readiness. Produces a deterministic COUNCIL report with what/why/how for epic-define (autonomous mode). Use when deciding what epic to build next.
---
...
4. **Dispatch all 8 advisor sub-agents IN PARALLEL.** Use the **Fast / Efficient Model** tier. Each sub-agent runs independently with its own fresh context window. The orchestrator provides ONLY the following in each sub-agent's prompt:
   ...
   **Sub-agent prompt template (Round 1):**
   ...
   Read your advisor guidelines: .agents/skills/epic-prioritize/references/advisor-[NAME].md
   Read the scoring rubric: .agents/skills/epic-prioritize/references/scoring-rubric.md
   Read the output template: .agents/skills/epic-prioritize/references/assessment-template.md
...
6. **Dispatch a Moderator sub-agent** using the **Advanced Reasoning Model** tier, with all 8 Round 1 assessments as input:
   ...
   **Moderator prompt (Phase A — Merge):**
   ...
   Read your guidelines: .agents/skills/epic-prioritize/references/moderator.md
