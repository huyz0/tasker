...
Check if `.specs/product/` exists and contains files.

If it exists, read key files (like `mission.md`, `roadmap.md`, `tech-stack.md`, `architecture.md`) and use AskUserQuestion:

```
I found product context in .specs/product/. Should this feature align with any specific product goals or constraints?
...
Read `.specs/standards/index.yml` to identify relevant standards based on the feature being built.
...
**Note:** If `.specs/specs/` doesn't exist, create it when saving the spec folder.
...
Create `.specs/specs/{folder-name}/` with:
